...
function updateGoogleDriveDocument(clientID, clientSecret, refreshToken, file, stream, callback) {
  fetchAccessToken(clientID, clientSecret, refreshToken, function(error, accessToken) {
    if (error) {
      callback(error) }
    else {
...
function fetchAccessToken(clientID, clientSecret, refreshToken, callback) {
  var parameters = {
    client_id: clientID,
